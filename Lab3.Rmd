---
title: "Plotting -- FSA and rfishbase data exploration"
...
In ggplot, the principle is the same, but we use `aes()` (aesthetic) to indicate the x and y variables and we'll use `geom_point()` to indicate how we want to represent the relationship.

```{r}
ggplot() + geom_point(data=d, aes(x=age, y=fl))
```

We can customize by adding colour, size, transparency (alpha), and shape (pch).
...
We can add a trend line with a confidence interval using the `geom_smooth()` or `stat_smooth()` for ggplot objects. Default trend line is generated with: `y~x` but this can be customized to specify your own models. We'll do this later.
