...
```{r}
prio_growth = popgrowth(Species="Prionace glauca")
prio_mat = maturity(Species="Prionace glauca")
```

```{r}
prio_lifespan = mean(prio_growth$tmax, na.rm=T)
prio_agemat = mean(prio_mat$AgeMatMin, na.rm=T)
paste0("Then.etal.2014 Natural Mortality rate: ", round(getMtmax(prio_lifespan), 3))
paste0("Dulvy.etal.2014 Natural Mortality rate: ", round(getMDulvy.etal.2014(prio_lifespan, prio_agemat), 3))
```
